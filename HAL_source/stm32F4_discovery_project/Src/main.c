...
int main(void)
{
  ...
  ILI9341_Init();  //Configure LCD, use fsmc. 2020.1.7 hlh
//  Key_Init();   //button init, use to stop camera.   2020.3.4 

//  MX_DMA_Init();	//this part must before than uart initlize.
  MX_Usart1Init();
  
  FSMC_SRAM_Init();         //use extern sram
  
//  RGB_DMA_Init();	//add dma to trans rgb data
  
//  tp_dev.init(); //touch pad init	2020.5.26
...
//    if(camera_state == Camera_On)
//  	{
//		MX_USB_HOST_Process();
//	}
//    else if(camera_state == Camera_Off)
//   	{
//		USB_DeinitCameraConnect();
//		uvc_frame_cnt = 0;
//                uvc_parameterDeinit();
//	}
...
//		HAL_UART_Transmit_DMA(&husart1, uvc_ready_framebuffer_ptr, uvc_ready_frame_length);
      ...
      if(frame_received_flag == 0)
      {
		...
		lcd_switch_buffer();
		LCD_Clear(WHITE);
		ILI9341_DispString_EN(0, 60,"Test UVC Host");
		ILI9341_DispString_EN(0, 80,"use 176*144");
      }
	  ...
	  {
		LCD_DrawImage(0, 100, (uint8_t*)uvc_ready_framebuffer_ptr);     //draw frame picture
	  ...
	  else
	  {
		...
		uint8_t tmp_str[32];
		sprintf((char*)tmp_str, "FPS: %d", last_fps);
		ILI9341_DispString_EN(10, 280,tmp_str);
		lcd_switch_buffer();
