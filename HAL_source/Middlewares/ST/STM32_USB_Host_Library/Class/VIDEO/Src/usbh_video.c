...
USBH_ClassTypeDef  VIDEO_Class2 = 
{
  "VIDEO",
  CC_VIDEO,
  USBH_VIDEO_InterfaceInit,
...
//        if (ep_size == 512)
    ...
    for(uint16_t i=0; i<phost->device.CfgDesc.wTotalLength; i++)        //use debug, printf cfg
    {
      printf("%02x ", phost->device.CfgDesc_Raw[i]);
    }
    printf("\n\r");
    ...
    {       
      ...
      USBH_OpenPipe(phost,
                    ...
                    phost->device.speed,
                    USB_EP_TYPE_ISOC,
                    VIDEO_Handle->camera.EpSize);//WARNING! 1000
