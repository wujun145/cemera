...
{			
	digitalL ( ILI9341_RST_PORT, ILI9341_RST_PIN );	 //低电平复位

	ILI9341_Delay ( 0xAFF ); 					   

	digitalH ( ILI9341_RST_PORT, ILI9341_RST_PIN );		 	 
...
void LCD_Clear(uint16_t color)
{
	...
	ILI9341_FillColor ( 320 * 240, color );		
}
...
//  HAL_SRAM_Write_DMA(&hsramMy, (uint32_t*)BANK1_SRAM3_ADDR, (uint32_t*)lcd_current_framebuffer, LCD_BUFFER_SIZE);
//	HAL_SRAM_Write_DMA(&hsramMy, (uint32_t*)BANK1_SRAM3_ADDR, (uint32_t*)pTestStr, 13);
}
/**
...
void ILI9341_Write_Color(uint16_t sx, uint16_t sy,uint16_t color)
{
		ILI9341_OpenWindow(sx,sy,sx+1,sy);
		ILI9341_Write_Cmd ( CMD_SetPixel );	
		ILI9341_Write_Data ( color );
}
extern DMA_HandleTypeDef hdma_memtomem_dma1;
...
{    
	...
	uint16_t i=0;
//	ILI9341_OpenWindow(sx, sy, LCD_PIXEL_WIDTH-1, UVC_TARGET_HEIGHT-1);
	...
	for(y=0; y<UVC_TARGET_HEIGHT; y++)
	{
		...
		for(x=0; x<UVC_TARGET_WIDTH; x+=2)
        {
            ...
            ILI9341_Write_Color(sx+x,sy+y,*image_ptr);		//写入数据
            ILI9341_Write_Color(sx+x+1,sy+y,*(image_ptr+1));	//写入数据
            image_ptr+= 2;//2 pixels
        }
	}
	
//	MySramWriteBuffer((uint8_t *)image_ptr, UVC_MAX_FRAME_SIZE * 2, strlen((uint8_t *)image_ptr));
	
//	if(HAL_DMA_Start_IT(&hdma_memtomem_dma1, RGB_BUFFER, LCD->LCD_RAM, strlen((uint8_t*)image_ptr)) != HAL_OK )
//	{
//		Error_Handler();
//	}
