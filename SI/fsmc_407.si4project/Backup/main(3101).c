...
static uint32_t tempBuf[5];
static uint32_t srcbuf[5]={'h','e','l','l','o'};
...
//        __close(f1);
        uint8_t *pDstBuffer;
		HAL_SRAM_Read_DMA(&hsramMy, (uint32_t*)BANK1_SRAM3_ADDR, (uint32_t *)pDstBuffer, 13);
		HAL_UART_Transmit_DMA(&husart1, pDstBuffer, 13);
//		HAL_DMA_Start_IT(&hdma_memtomem_dma2, (uint32_t)&srcbuf, (uint32_t)&tempBuf, 5);
      }
