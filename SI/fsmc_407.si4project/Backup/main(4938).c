...
#define tempBufAddress      (uint32_t)0x68000000
...
uint8_t *pData = "helloWorld hellothe solor system\r\n";
...
//        __close(f1);
//		HAL_UART_Transmit_DMA(&husart1, uvc_ready_framebuffer_ptr, strlen(uvc_ready_framebuffer_ptr));
		HAL_DMA_Start_IT(&hdma_memtomem_dma2, (uint32_t)uvc_ready_framebuffer_ptr, tempBufAddress, strlen(uvc_ready_framebuffer_ptr));
      }
...
//		HAL_UART_Transmit(&husart1, pData, strlen(pData), 0xffff);
//		HAL_UART_Transmit_DMA(&husart1, pData, strlen(pData));
	}
  }
